...
while true
    ...
    for k = 1: numel(xlf)
        ...
        if (seq.frame == 1)
            %f_pre_f{k} = zeros(size(model_xf{k}));
            model_xf{k} = xlf{k};
            %mu = 0;
        else
            %mu = temporal_regularization_factor(k);
            model_xf{k} = ((1 - 0.01) * model_xf{k}) + (0.01 * xlf{k});
        end
...
%             T = prod(use_sz);
            ...
            while (i <= admm_iter)
                ...
                g_f = (((1/(T*mu)) * bsxfun(@times, yf{k}, model_xf{k})) - ((1/mu) * l_f) + h_f) - ...
                    bsxfun(@rdivide,(((1/(T*mu)) * bsxfun(@times, model_xf{k}, (S_xx .* yf{k}))) - ((1/mu) * bsxfun(@times, model_xf{k}, S_lx)) + (bsxfun(@times, model_xf{k}, S_hx))), B);

                %   solve for H
                h = (T/((mu*T)+ admm_iter))* ifft2((mu*g_f) + l_f);
                [sx,sy,h] = get_subwindow_no_window(h, floor(filter_sz_cell{k}/2) , small_filter_sz{k});
                %t = single(zeros(filter_sz_cell{k}(1), filter_sz_cell{k}(2), size(h,3)));
