...
for frame = 1:num_frames,
    ...
    if frame > 1
        ...
        response = real(ifft2(alphaf_nu .* kf ./ (alphaf_de+1e-4)));
    ...
    new_alphaf_nu = yf .* kf;
    new_alphaf_de = kf .* (kf + lambda);
    
    if frame == 1
        % first frame, train with a single image
        alphaf_nu = new_alphaf_nu;
        alphaf_de = new_alphaf_de;
    else
        % subsequent frames, update the model
        alphaf_nu = (1 - learning_rate) * alphaf_nu + learning_rate * new_alphaf_nu;
        alphaf_de = (1 - learning_rate) * alphaf_de + learning_rate * new_alphaf_de;
    end
