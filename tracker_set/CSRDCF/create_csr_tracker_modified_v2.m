function tracker = create_csr_tracker_modified_v2(img, init_bbox, visualization, init_params)
% 表示缺省输入第四个参数
if nargin < 4
...
seg_colorspace = init_params.seg_colorspace;     % 'rgb' or 'hsv' 初始为HSV
use_segmentation = init_params.use_segmentation;  % false to disable use of segmentation 开始状态为true
...
img0 = bsxfun(@minus, double(img), mean(img,3)); % 直接减去一般的范围
...
target_dummy_mask(y0:y1, x0:x1) = 1; % 把中心部分置为1
target_dummy_mask = single(target_dummy_mask);
% area = (obj_size(1) + 1)(obj_size(2) +1)
target_dummy_area = sum(target_dummy_mask(:));

[mask,hist_fg,hist_bg] = ...
    create_spatial_matrix_v2(img, bb, c, Y,...
...
% create filter - using segmentation mask，学习一下方法！单独运算直接得出结果
...
if visualization
    ...
    %     rect_positionglb =  [bb([1,2]), bb([3,4])];
    rect_positionglb = bb;
