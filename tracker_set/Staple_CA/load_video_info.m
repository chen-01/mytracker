...
function seq = load_video_info_UAV123(video_name, video_path, ground_truth_path, type)
...
%     img_files = {img_files.folder,img_files.name};
    seq.s_frames_temp = cellstr(img_files);
    seq.s_frames = (seq.s_frames_temp(1, seq.st_frame : seq.en_frame))';
%     N = size(seq.s_frames);
%     n = N(1,1);
%     for j=1:n
% %     for j = seq.st_frame:seq.en_frame
%         seq.s_frames{j} = [img_path '\' seq.s_frames{j}];
%     end