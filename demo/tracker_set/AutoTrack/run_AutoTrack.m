...
function results = run_AutoTrack(seq)
    ...
    %  Global feature parameters
    % 特征提取时使用的单元格大小，这里设置为4像素。
    % 单元格大小在特征提取过程中扮演着重要角色，它决定了从图像中提取特征的区域大小。
    % 较小的单元格可以捕捉到更精细的图像细节，但可能会增加计算复杂度；
    % 较大的单元格则可以减少计算量，但可能会忽略一些细节
    ...
    % 初始化你新增的控制变量
    gallery = {}; % 关键帧特征库
    max_gallery_size = 5; % 特征库上限
    fb_threshold = 15; % 反向校验欧氏距离阈值（像素）
    spr_threshold = 0.75; % SPR触发阈值
    is_lost = false; % 丢失状态位
    velocity = [0, 0]; % 运动模型速度向量
